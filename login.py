    ...
    def attemptLogin(): #attempt login function
        ...
        try:
            ...
            if attempt == True: #if log in success
                ...
                openMenu(username) #open menu window passing in dev credentials
            else:
                errorLabel.config(text=attempt) #if login fail, display reason
        except AuthError as e:
            print("Login Error", str(e)) #if login error, display error
...
def createUser(loginWin, OpenMenu): #open create new user window function
    ...
    rule3text = tk.Label(createUserWin, text="- Contain a special character", font=textFont, fg="grey") #sets password rule 2 label
