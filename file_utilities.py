...
def loadFile(username,filename): #load file function
    try:
        ...
        try:
            data = bytes.fromhex(hexdata) #attempt to transform hex data to bytes
        except ValueError:
...
def moveFile(oldFile, newFile): #move file function
    try:
        ...
        raise FileWriteError (f"File Move Error: {e}") #displays error if fail

def deleteFile(path): #delete file function
    try:
        safeDelete(path) #attempt to delete file
    except DeleteFileError as e:
        raise DeleteFileError(f"File Delete Error: {e}") #displays error if fail
