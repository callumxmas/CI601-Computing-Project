...
def openMenuWin(username, key): #opens main menu window function
    ...
    def recentlyDeletedPopUp(): #bin pop up window
        ...
        def loadBinFiles(): #function to load file found in bin
            ...
            loadBinFiles.binFiles = binFiles #makes the bin list accessible to other bin function
            ...
            for f in binFiles: #if bin no empty
                raw = f.name[:-4] #raw hold file name (including time stable) without extension
        ...
        def recover(): #recover file function
            ...
            while checkFileName(attempt[:-4]): #loops though counter, checking if a file matches with file trying to be recovered
                attempt = f"{raw} {num}.txt" #if match found, adds number to filename
                num += 1 #incrempt counter

            try:
                file_utilities.moveFile(f"./users/{username}/bin/{selectedFile}", f"./users/{username}/{attempt}") #attempts to move file back
            except FileWriteError as e:
                notification(str(e)) #displays error if fail
                return #skip
